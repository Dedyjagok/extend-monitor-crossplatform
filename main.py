    ...
    try:
        with mss.mss() as sct:
            ...
            if len(sct.monitors) < 3:
                print("[ERROR] Monitor Virtual tidak terdeteksi! Pastikan IddSampleDriver aktif & mode Extend nyala.")
                target_monitor = sct.monitors[1] # Fallback ke layar utama (Mirroring)
            else:
                print("[INFO] Mengirim tampilan Monitor 2 (Extended)...")
            ...
            while True:
                ...
                # "L" adalah unsigned long (4 bytes)
                message_size = struct.pack("L", len(data)) 
                
                client_socket.sendall(message_size + data)

                # Tekan 'q' di server untuk stop (biasanya tidak perlu)
